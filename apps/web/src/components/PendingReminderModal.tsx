...
export const PendingReminderModal = ({
  ...
  const getAssigneeName = (userId: string) => {
    return project?.project_members?.find(m => m.user_id === userId)?.users.full_name || 'Unknown';
  };
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto min-h-0">
                    ...
                    if (match) {
                      return (
                        <>
                          ...
                          <span className="text-slate-700 font-medium truncate">{match[2]}</span>
                        </>
                      );
                    }
                    return <span className="text-slate-700 font-medium">{task.title}</span>;
                  })()}
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              {selectedAssigneeIds.map(id => (
                ...
                >
                  <span className="text-xs font-bold uppercase tracking-wider">{getAssigneeName(id)}</span>
                  <button 
          ...
          <div className="space-y-3">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-4 focus:ring-accent/5 transition-all resize-none h-24"
            />
          </div>
        </div>
