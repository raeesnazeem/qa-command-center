...
export const TaskCommentsModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md rounded-3xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
        {/* Header */}
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className={`text-[10px] font-bold uppercase ${
                    item.type === 'rebuttal' ? 'text-red-600' : 'text-slate-500'
                  }`}>
                ...
                {item.screenshot_url && (
                  <div className="mt-2 rounded-lg overflow-hidden border border-slate-200">
                    <img src={item.screenshot_url} className="w-full h-auto" alt="Screenshot" />
                  </div>
                )}
              </div>
            ))
