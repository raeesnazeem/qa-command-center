...
export const DashboardPage = () => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return data.all_projects.filter(p => 
      ...
      <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12 px-4">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-2 font-medium">You have <span className="text-indigo-600 font-bold">{data?.my_open_tasks ?? 0}</span> tasks assigned to you across projects.</p>
          </div>
          <Link to="/tasks" className="btn-unified-primary flex items-center gap-2 group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data?.my_tasks.length === 0 ? (
                <div className="md:col-span-2 bg-white border-2 border-dashed border-slate-200 rounded-3xl p-12 text-center">
                  ...
                  <p className="text-slate-500 text-sm mt-1">No open tasks currently assigned to you.</p>
                </div>
              ) : (
                data?.my_tasks.map((task) => (
                  ...
                  >
                    ...
                    <h4 className="font-black text-slate-900 text-lg mb-2 group-hover:text-accent transition-colors line-clamp-2 leading-tight">{task.title}</h4>
                    <p className="text-xs text-slate-500 font-medium mb-6 line-clamp-2">{(task as any).projects?.name}</p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 rounded-3xl p-6 text-white space-y-6 shadow-xl">
              <div>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Total Assigned</p>
                <p className="text-3xl font-black">{data?.my_tasks.length ?? 0}</p>
              </div>
              <div className="pt-6 border-t border-white/10">
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Open Issues Count</p>
                <p className="text-3xl font-black text-amber-400">{data?.open_issues ?? 0}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    return (
      <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12 px-4">
        <header>
          ...
          <p className="text-slate-500 mt-2 font-medium italic">Welcome back. Prioritize pre-release audits or search for specific projects.</p>
        </header>
              ...
              {data?.pre_release_projects?.map((project) => (
                ...
                >
                  <div className="absolute top-0 right-0 p-3">
                    <Zap size={16} className="text-amber-500 fill-amber-500 opacity-20 group-hover:opacity-100 transition-opacity" />
                  </div>
                  <h4 className="font-black text-slate-900 text-xl mb-1 group-hover:text-accent transition-colors leading-tight">{project.name}</h4>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-50">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Active Runs</span>
                      <span className="text-sm font-black text-slate-900">{project.qa_runs?.filter((r: any) => r.status === 'running').length || 0}</span>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data?.post_release_projects?.map((project) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-50">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Open Issues</span>
                      <span className="text-sm font-black text-slate-900">{project.open_issues_count || 0}</span>
                    </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Project Database</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredProjects.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-8 py-12 text-center text-sm text-slate-400 font-medium italic">
                    ...
                    <tr key={project.id} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-3">
                          <div className={`w-2 h-2 rounded-full ${project.is_pre_release ? 'bg-amber-400' : 'bg-slate-300'}`} />
                          <Link to={`/projects/${project.id}`} className="text-sm font-black text-slate-900 group-hover:text-accent transition-colors">
                            {project.name}
                          </Link>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-5 text-xs text-slate-400 font-medium">
                        {project.qa_runs?.[0] ? format(new Date(project.qa_runs[0].created_at), 'MMM d, yyyy') : 'No runs yet'}
                      </td>
                      <td className="px-8 py-5">
                        <Link to={`/projects/${project.id}`} className="text-accent hover:text-black transition-colors">
    ...
    <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700 pb-12 px-4">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Global intelligence across all organizational projects.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link to="/projects" className="btn-unified-secondary h-11 flex items-center gap-2">
            <Layers size={18} />
            Browse Projects
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group">
            <div className="flex items-center justify-between mb-4">
              <div className={`p-3 ${stat.bg} rounded-2xl group-hover:scale-110 transition-transform`}>
                <stat.icon className={`w-5 h-5 ${stat.color}`} />
              </div>
            </div>
            <div className={`text-4xl font-black ${stat.label === 'Open Issues' && (data?.open_issues ?? 0) > 0 ? 'text-red-600' : 'text-slate-900'}`}>
              {stat.value}
            </div>
            <div className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-2">{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data?.pre_release_projects?.map((project) => (
            <div key={project.id} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group relative flex flex-col">
              ...
              <Link to={`/projects/${project.id}`} className="flex-1">
                <h4 className="font-black text-slate-900 text-lg mb-1 group-hover:text-accent transition-colors">{project.name}</h4>
                <p className="text-xs text-slate-400 font-medium mb-6">{project.client_name || 'Internal'}</p>
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-50">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Runs: {project.qa_runs?.filter((r: any) => r.status === 'running').length || 0}</span>
          ...
          {data?.post_release_projects?.map((project) => (
            <div key={project.id} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group relative flex flex-col">
              ...
              <Link to={`/projects/${project.id}`} className="flex-1">
                ...
                <p className="text-xs text-slate-400 font-medium mb-6">{project.client_name || 'Internal'}</p>
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-50">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Open Issues: {project.open_issues_count || 0}</span>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {data?.recent_runs.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-8 py-12 text-center text-sm text-slate-400 font-medium italic">No recent runs initiated</td>
                  </tr>
                ) : (
                  data?.recent_runs.map((run) => (
                    <tr key={run.id} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex flex-col">
                          <Link to={`/projects/${run.project_id}/runs/${run.id}`} className="text-sm font-black text-slate-900 group-hover:text-accent transition-colors leading-tight">
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center">
                          <div className={`w-2 h-2 rounded-full mr-3 ${
                            ...
                            run.status === 'running' ? 'bg-accent animate-pulse shadow-[0_0_8px_rgba(var(--accent-rgb),0.4)]' : 
                            run.status === 'failed' ? 'bg-red-500' : 'bg-slate-300'
                          }`} />
                          <span className="text-xs font-black text-slate-600 uppercase tracking-tight">{run.status}</span>
                        </div>
                      </td>
                      <td className="px-8 py-5 text-xs text-slate-400 font-medium">
            ...
            <div className="bg-white border border-slate-100 rounded-3xl shadow-sm divide-y divide-slate-50 overflow-hidden">
              {data?.my_tasks.length === 0 ? (
                <div className="p-12 text-center">
                  <p className="text-xs text-slate-400 font-medium italic">No urgent tasks for you</p>
                </div>
              ) : (
                data?.my_tasks.map((task) => (
                  <Link key={task.id} to={`/tasks?taskId=${task.id}`} className="p-6 hover:bg-slate-50 transition-colors block group">
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1">
                        <p className="text-sm font-black text-slate-900 group-hover:text-accent transition-colors leading-tight line-clamp-2">{task.title}</p>
                        <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest">{(task as any).projects?.name}</p>
                      </div>
            ...
            <div className="bg-amber-50/50 border border-amber-100 rounded-3xl p-6 space-y-4">
              {data?.pending_signoffs.length === 0 ? (
                <div className="text-center py-4">
                  <p className="text-xs text-emerald-600 font-black uppercase tracking-widest">All runs verified</p>
                </div>
              ) : (
                data?.pending_signoffs.map((run) => (
                  <div key={run.id} className="flex items-center justify-between bg-white p-4 rounded-2xl border border-amber-100 shadow-sm hover:shadow-md transition-all group">
                    <div className="flex-1 min-w-0 mr-4">
                      <p className="text-xs font-black text-slate-900 truncate uppercase tracking-tight">{(run as any).projects?.name}</p>
                      <p className="text-[9px] text-slate-400 font-bold mt-1">Ready for review • {format(new Date(run.completed_at!), 'MMM d')}</p>
                    </div>
                    <Link 
