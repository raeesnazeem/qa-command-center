...
export interface ActivityAction {
  type: string;
  details?: any;
  isAdminOnly?: boolean; // [Step 2.11] Special group for Admin-only activities
}
...
export async function logActivity(
  ...
  try {
    // [Step 2.3] Save the action to the activity_logs table
    ...
    // [Step 2.4] Get the activity_id from the newly saved row
    activityId = data.id;
  } catch (error) {
  ...
  // [Step 2.10] Use a separate Try-Catch for notifications so main action still works
  try {
    // [Step 2.11] If it's Admin Only, skip the notifications table
