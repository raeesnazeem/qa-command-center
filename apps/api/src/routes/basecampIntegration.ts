 ...
 */
router.post('/:id/basecamp', clerkAuth, requireRole('qa_engineer'), async (req: Request, res: Response) => {
  const { id } = req.params;

  try {
    // 1. Load task + finding + project settings
    ...
    if (!projectSettings || !projectSettings.basecamp_token || !projectSettings.basecamp_account_id || !projectSettings.basecamp_project_id || !projectSettings.basecamp_todolist_id) {
      return res.status(400).json({ error: 'Basecamp integration not configured for this project' });
    }
...
router.post('/basecamp', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (kind === 'todo_completion') {
      ...
      if (findError || !task) {
        logger.info(`Basecamp todo completed, but no matching task found for ID: ${basecampTaskId}`);
