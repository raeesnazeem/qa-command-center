...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!todolistId) {
        todolistId = task.projects.is_post_release ? projectSettings.basecamp_post_todolist_id : projectSettings.basecamp_todolist_id;
      }
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (todolistId) {
         ...
         const mentionsList = await Promise.all((userList || []).map(async (u) => {
           if (u.basecamp_person_id) {
             try {
               const person = await getBasecampPerson(projectSettings!.basecamp_token, projectSettings!.basecamp_account_id, Number(u.basecamp_person_id));
               if (person?.attachable_sgid) return formatBasecampMention(person.attachable_sgid, person.name);
             } catch (err) {
               console.error(`Failed to get Basecamp person ${u.basecamp_person_id}`, err);
             }
           }
           return null;
         }));
