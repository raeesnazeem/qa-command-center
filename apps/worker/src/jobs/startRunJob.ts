...
export async function processStartRunJob(job: Job) {
  ...
  try {
    ...
    if (insertedPages) {
        ...
        if (chunk.length === 1) {
          ...
          return {
            ...
            opts: {
              attempts: 3,
              backoff: { type: 'exponential', delay: 5000 },
              lockDuration: 600000 // 10 minutes for batches
            }
          };
        }
      });
      ...
      logger.info({ runId, jobCount: jobs.length }, 'Enqueued scan jobs successfully');
    }
