...
async function main() {
  ...
  const groupsUrl = "https://3.basecampapi.com/4023059/buckets/42822223/todolists/9899445368/groups.json"
  const response = await axios.get(groupsUrl, { headers })
  console.log("Found groups:")
  for (const group of response.data) {
    console.log(`- Group: "${group.title}" (ID: ${group.id})`)
    const groupTodosUrl = group.todos_url
    const todosResponse = await axios.get(groupTodosUrl, { headers })
    for (const todo of todosResponse.data) {
