...
async function main() {
  ...
  const messageBoardTool = bucketResponse.data.dock?.find(
    (tool: any) => tool.title === "Message Board" || tool.url?.includes("/message_boards/"),
  )
  ...
  const orderDetailsMsg = messagesResponse.data.find((msg: any) =>
    (msg.subject || msg.title || "").toLowerCase().includes("project order details"),
  )
  ...
  console.log(msgResponse.data.content)
  ...
  for (const comment of commentsResponse.data) {
    console.log(`- Comment from ${comment.creator.name}:`)
